...
const updateConfigCache = async () => {
    try {
        ...
        configCache.blockAds = items.blockAds ?? false;
        configCache.trustedUsers = new Set(items.trustedUsers ?? []);
        configCache.badExamples = items.badExamples ?? [];
 ...
 */
const checkIsAd = (article) => {
    // 1. 属性ベースのチェック
    if (article.querySelector(PROMO_SELECTOR))
        return true;
    // 2. テキストベースのチェック
 ...
 * 学習機能: NG例として登録
 */
const registerBadExample = async (text) => {
 ...
 * 信頼機能: ホワイトリスト登録
 */
const registerTrustedUser = async (username, article) => {
 ...
 */
const applyScanningStyle = (article) => {
    if (article.dataset.vibeStatus)
        return;
...
const applyBlockStyle = (article, reason) => {
  ...
  `;
    // イベント設定
    blockCurtain.querySelector('#btn-reveal').onclick = (e) => {
        e.stopPropagation();
        if (confirm('一時的に表示しますか？'))
            blockCurtain.remove();
    };
    blockCurtain.querySelector('#btn-learn').onclick = (e) => {
        e.stopPropagation();
        registerBadExample(text);
    };
    if (username) {
        blockCurtain.querySelector('#btn-trust').onclick = (e) => {
            e.stopPropagation();
            registerTrustedUser(username, article);
        };
    }
        ...
        mutation.addedNodes.forEach((node) => {
            if (node instanceof HTMLElement) {
                if (node.matches(TWEET_SELECTOR))
                    processTweet(node);
                node.querySelectorAll(TWEET_SELECTOR).forEach((t) => processTweet(t));
            }
        });
    });
});
